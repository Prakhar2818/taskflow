    ...
    <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-6 shadow-xl border border-purple-200">
      ...
      <div className="space-y-2">
        <h4 className="font-semibold text-purple-700 mb-3 flex items-center gap-2">
          <span>📋</span>x1 
          Session Tasks ({activeSession.completedTasks || 0}/{activeSession.tasks.length})
        </h4>
        <div className="max-h-48 overflow-y-auto space-y-2">
          {getTaskProgressInSession().map((task, index) => (
            <div
              key={index}
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-gray-500">{task.duration || 0}min</span>
                    <span>{getPriorityEmoji(task.priority)}</span>
                  </div>
                </div>
                {task.isCurrent && taskIsRunning && totalTime > 0 && (
                  <div className="mt-1">
                    <div className="bg-blue-200 rounded-full h-1">
                      <div 
                        className="bg-blue-500 h-1 rounded-full transition-all duration-1000"
                        style={{ 
                          width: `${((totalTime - remainingTime) / totalTime) * 100}%` 
                        }}
                      ></div>
                    </div>
                  </div>
